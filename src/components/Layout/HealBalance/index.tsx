...
// Components
import HealBalanceStatsTable from "./HealBalanceStatsTable"
// Ui
...
// Styles and types
import { HealBalanceProps } from "./types"

/**
 * Renders the HealBalance component, which displays the total heal given and received
 * for an active game, along with a button to show detailed stats. Allows to open a modal
 * to save a new heal given or received.
 *
...
function HealBalance({
  ...
  const calculatedClassNames = cx(
    twMerge(
      "heal-balance min-w-full flex justify-center items-center",
      className
    )
  )
  const utils = trpc.useUtils()
  const { data: balance, isLoading: balanceLoading } =
    trpc.stats.heal.game.useQuery(undefined, {
      enabled: !!characterId && !!gameId
    })
  const { data: stats, isLoading: statsLoading } =
    trpc.stats.heal.table.useQuery(undefined, {
      enabled: !!characterId
    })
  const [isShowDetails, setIsShowDetails] = useState(false)
  const openModal = useStore((state) => state.openModal)
  const handleDataFetch = () => {
    utils.stats.heal.game.invalidate()
    utils.stats.heal.table.invalidate()
  }
  const handleSaveHeal = (
  ...
  return (
    <Stack>
        ...
        <div className="relative p-4 flex w-full min-w-fit justify-center items-center h-32 rounded-md shadow-sm shadow-block-600 bg-block-700">
          ...
          <div className="flex flex-col flex-1 h-full content-center items-center px-4 relative">
            <Stack className="items-center" gap="tight">
              <Title>Heal</Title>
              <Text size="small">Given and Received</Text>
            </Stack>
            <Button
      ...
      <HiddenRoom isShown={isShowDetails}>
        <HealBalanceStatsTable className="w-full" stats={stats || []} />
      </HiddenRoom>
    </Stack>
  )
}
...
/**
 * Renders an interactive button representing a heal statistic, displaying a count and an icon
 * (hospital for positive/given, heart for negative/received).
 *
 * @param count - The numeric value to display inside the button.
 * @param isPositive - Determines the icon and style; true for positive (given), false for negative (received).
 * @param onClick - Callback invoked when the button is clicked, receiving the mouse event and the `isPositive` flag.
 */
function HealStatButton({
  count,
  isPositive,
  onClick
}: {
  ...
  return (
    ...
    >
      ...
      <div className="absolute z-1 top-0 left-0 w-full h-full rounded-2xl bg-block-900 opacity-5 p-2">
        <SmartImage
          alt="heal"
          src={`/public/images/${isPositive ? "heal" : "healed"}.svg`}
        />
      </div>
    </motion.div>
  )
